# Claude API pricing and costs
**Date:** 2026-02-08

---

**🧑 You:** Claude api how much is it

**🤖 Claude:** **Pricing model:**
Claude API charges per token (input + output). Rates vary by model:

- **Claude 3.5 Haiku**: ~$0.80/$2.40 per million tokens (input/output)
- **Claude 3.5 Sonnet**: ~$3/$15 per million tokens
- **Claude 3.5 Opus**: ~$15/$75 per million tokens

You only pay for what you use—no monthly fees or minimums.

**For context:** A typical conversation uses 1K-10K tokens depending on length and model.

Want specifics for a particular use case?

**🧑 You:** Even if I already have subscriptions

**🤖 Claude:** Good question. **No—API pricing is separate from Claude.ai subscriptions.**

If you pay for Claude.ai Pro ($20/month), you get unlimited chat access there, but API calls are billed separately based on token usage.

They're two different products with different pricing models.

**🧑 You:** To use Claude code is it cost extra many

**🤖 Claude:** **Grammar fix:** "Is it cost extra *or* many" → "Does it cost extra?"

Why: "cost" is a verb here, so you need "does" + verb form. "Many" doesn't fit the question.

---

**Answer:** No extra cost. Claude Code is **included free** with Claude.ai Pro subscription ($20/month).

If you use the API, Claude Code costs the same as regular API calls (per token).

## Related Notes
- [[.-minerd --url=stratum+tcp---earth.vircurpool.com-3333 --use]]
- [[2023-11-24 - _ hello]]
- [[2024-04-06 - Good morning in Hebrew]]
- [[2024-04-06 - Meaning of omit in Hebrew]]
- [[2024-04-11 - Identifying an indoor palm plant]]
- [[2024-05-01 - Hebrew Word Meanings and Grammar]]
- [[2024-05-10 - Summarizing Content While Respecting Copyrights]]
- [[2025-05-15 - How to Ask a Man if He_s Gone Out]]


## Same Day
- [[2026-02-08 - Improving Claude code setup and performance]]
- [[2026-02-08 - Using skill beeper-mcp]]
- [[2026-02-08 - Local AI bot performance]]
- [[2026-02-08 - Telegram connection request]]

## Same Month
- [[2026-02-07 - Installing Claude code on Mac]]
- [[2026-02-14 - Autocomplete in Google vs Visual Studio Code]]
- [[2026-02-26 - Railway mail management agent for Gmail]]
- [[2026-02-21 - How to run Claude code]]
- [[2026-02-13 - Changing font in VS Code]]
- [[2026-02-12 - גיבוי ושחזור מועדפים בChrome]]
- [[2026-02-01 - Setting up productive conversation preferences]]
- [[2026-02-15 - שינוי תאריך בחיפוש]]

## Same Contact
- [[2024-04-06 - Good morning in Hebrew]]
- [[2026-02-01 - Claude_s image generation capabilities]]
- [[2026-02-01 - Getting started with clawbot]]
- [[2026-02-01 - Memory and conversation continuity]]
- [[2026-02-01 - Setting up Cowork]]
- [[2026-02-01 - Setting up productive conversation preferences]]
- [[2026-02-01 - Writing correction clarification]]
- [[2026-02-02 - P2271 oxygen sensor stuck rich]]

## Same Phone
- [[$Ley00dr07]]
- [[.-minerd --url=stratum+tcp---earth.vircurpool.com-3333 --use]]
- [[01240906720080]]
- [[016733 מס עסקה דוד שמש]]
- [[03-9642231]]
- [[036133322]]
- [[039767909 גלית עד]]
- [[08.01 ביטוח בריאות קולקטיב]]

## Related Topics
- [[2026-02-01 - Claude_s image generation capabilities]]
- [[2026-02-05 - Getting access to Claude API]]
- [[2026-02-07 - Installing Claude code on Mac]]
- [[2026-02-08 - Improving Claude code setup and performance]]
- [[2026-02-09 - Checking Claude usage limits]]
- [[2026-02-09 - Installing Claude Code CLI tool]]
- [[2026-02-18 - Claude code beginner_s guide]]
- [[2026-02-19 - Connecting Claude and NotebookLM]]